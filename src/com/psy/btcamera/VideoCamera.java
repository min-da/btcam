...
package com.psy.btcamera;

import com.psy.btcamera.ui.CameraPicker;
import com.psy.btcamera.ui.IndicatorControlContainer;
import com.psy.btcamera.ui.IndicatorControlWheelContainer;
import com.psy.btcamera.ui.PopupManager;
import com.psy.btcamera.ui.Rotatable;
import com.psy.btcamera.ui.RotateImageView;
import com.psy.btcamera.ui.RotateLayout;
import com.psy.btcamera.ui.RotateTextToast;
import com.psy.btcamera.ui.SharePopup;
import com.psy.btcamera.ui.ZoomControl;
 ...
 */
public class VideoCamera extends ActivityBase implements
	...
	public void onCreate(Bundle icicle) {
		...
		Thread startPreviewThread = new Thread(new Runnable() {
			...
			public void run() {
				try {
					mCameraDevice = Util
							.openCamera(VideoCamera.this, mCameraId);
		...
		// The R.id.labels can only be found in phone layout. For tablet, the id
		// is
		// R.id.labels_w1024. That is, mLabelsLinearLayout should be null in
		// tablet layout.
	...
	private void initializeIndicatorControl() {
		...
		CameraPicker
				.setImageResourceId(R.drawable.ic_switch_video_facing_holo_light);
	...
	private class MyOrientationEventListener extends OrientationEventListener {
		...
		public void onOrientationChanged(int orientation) {
			...
			if (mOrientationCompensation != orientationCompensation) {
				...
				if (effectsActive()) {
					mEffectsRecorder
							.setOrientationHint(mOrientationCompensation % 360);
				}
	...
	private void readVideoPreferences() {
		...
		if (mEffectType != EffectsRecorder.EFFECT_NONE) {
			...
			if (mIndicatorControlContainer != null) {
				mIndicatorControlContainer.overrideSettings(
						CameraSettings.KEY_VIDEO_QUALITY, null);
			}
		}
	...
	private void setPreviewDisplay(SurfaceHolder holder) {
		try {
			if (effectsActive()) {
				mEffectsRecorder.setPreviewDisplay(mSurfaceHolder,
	...
	private void initializeEffectsPreview() {
		...
		boolean inLandscape = (getRequestedOrientation() == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
		...
		// TODO: Confirm none of the foll need to go to
		...
		mEffectsRecorder.setPreviewDisplay(mSurfaceHolder, mSurfaceWidth,
				mSurfaceHeight);
	...
	private void generateVideoFilename(int outputFileFormat) {
		...
		mCurrentVideoValues.put(MediaColumns.TITLE, title);
		mCurrentVideoValues.put(MediaColumns.DISPLAY_NAME, filename);
		mCurrentVideoValues.put(VideoColumns.DATE_TAKEN, dateTaken);
		mCurrentVideoValues.put(MediaColumns.MIME_TYPE, mime);
		mCurrentVideoValues.put(MediaColumns.DATA, mVideoFilename);
		...
		Location loc = mLocationManager.getCurrentLocation();
		if (loc != null) {
			mCurrentVideoValues.put(VideoColumns.LATITUDE, loc.getLatitude());
			mCurrentVideoValues.put(VideoColumns.LONGITUDE, loc.getLongitude());
		}
	...
	private void addVideoToMediaStore() {
		if (mVideoFileDescriptor == null) {
			...
			mCurrentVideoValues.put(MediaColumns.SIZE, new File(
			...
			if (duration > 0) {
				...
				mCurrentVideoValues.put(VideoColumns.DURATION, duration);
			} else {
	...
	private void addBaseMenuItems(Menu menu) {
		MenuHelper.addSwitchModeMenuItem(menu, ModePicker.MODE_CAMERA,
				new Runnable() {
					@Override
					public void run() {
	...
	private void showRecordingUI(boolean recording) {
		if (recording) {
			...
			mShutterButton
					.setBackgroundResource(R.drawable.btn_shutter_video_recording);
			mRecordingTimeView.setText("");
			mRecordingTimeView.setVisibility(View.VISIBLE);
			if (mReviewControl != null)
				mReviewControl.setVisibility(View.GONE);
			if (mCaptureTimeLapse) {
				if (Util.isTabletUI()) {
	...
	private void getThumbnail() {
		if (mCurrentVideoUri != null) {
			Bitmap videoFrame = Thumbnail.createVideoThumbnail(
					mCurrentVideoFilename, mPreviewFrameLayout.getWidth());
			if (videoFrame != null) {
	...
	private void showAlert() {
		Bitmap bitmap = null;
		if (mVideoFileDescriptor != null) {
			bitmap = Thumbnail.createVideoThumbnail(
		...
		if (bitmap != null) {
			...
			bitmap = Util.rotateAndMirror(bitmap,
					-mOrientationCompensationAtRecordStart, mirror);
		...
		if (mMediaRecorderRecording) {
			...
			try {
				if (effectsActive()) {
					...
					// because thumbnail
					// may not be ready. In such case addVideoToMediaStore is
					// called later
	...
	private void updateThumbnailButton() {
		if (mThumbnail == null
				|| !Util.isUriValid(mThumbnail.getUri(), mContentResolver)) {
	...
	private void setCameraParameters() {
		...
		if (isSupported(flashMode, supportedFlash)) {
			...
			if (flashMode == null) {
				flashMode = getString(R.string.pref_camera_flashmode_entry_external);//    pref_camera_flashmode_no_flash);
			}
		}
	...
	public void onCancelBgTraining(View v) {
		...
		// Tell the indicator controller to redraw based on new shared pref
		// values
		mIndicatorControlContainer.reloadPreferences();
	...
	private void restorePreferences() {
		...
		if (mIndicatorControlContainer != null) {
			mIndicatorControlContainer.dismissSettingPopup();
			CameraSettings.restorePreferences(this, mPreferences, mParameters);
			mIndicatorControlContainer.reloadPreferences();
			onSharedPreferenceChanged();
		}
	}
	...
	public void onSharedPreferenceChanged() {
		...
		synchronized (mPreferences) {
			...
			if (mCameraId != cameraId) {
				...
				Intent intent;
				if (mIsVideoCaptureIntent) {
					// If the intent is video capture, stay in video capture
					// mode.
					intent = getIntent();
				} else {
				...
				intent.putExtra(BACKGROUND_URI_GALLERY_EXTRA,
						mEffectUriFromGallery);
	...
	private void storeImage(final byte[] data, Location loc) {
		...
		if (uri != null) {
			// Create a thumbnail whose width is equal or bigger than that of
			// the preview.
			int ratio = (int) Math
					.ceil((double) mParameters.getPictureSize().width
							/ mPreviewFrameLayout.getWidth());
			int inSampleSize = Integer.highestOneBit(ratio);
			mThumbnail = Thumbnail.createThumbnail(data, orientation,
					inSampleSize, uri);
			if (mThumbnail != null) {
				mThumbnailView.setBitmap(mThumbnail.getBitmap());
			}
