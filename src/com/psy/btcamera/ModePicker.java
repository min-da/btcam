...
/**
 * A widget that includes three mode selections {@code RotateImageView}'s and a
...
public class ModePicker extends RelativeLayout implements View.OnClickListener,
	...
	public ModePicker(Context context, AttributeSet attrs) {
		...
		mFadeIn = AnimationUtils.loadAnimation(context,
				R.anim.mode_selection_fade_in);
		mFadeOut = AnimationUtils.loadAnimation(context,
				R.anim.mode_selection_fade_out);
	...
	protected void onFinishInflate() {
		...
		mModeSelectionIcon[MODE_VIDEO] = (RotateImageView) findViewById(R.id.mode_video);
		mModeSelectionIcon[MODE_CAMERA] = (RotateImageView) findViewById(R.id.mode_camera);
	...
	public void setEnabled(boolean enabled) {
		...
		if (mCurrentModeFrame != null)
			mCurrentModeFrame.setEnabled(enabled);
	...
	private void updateModeState() {
		...
		if (mCurrentModeFrame != null) {
			for (int i = 0, j = 0; i < MODE_NUM; ++i) {
				...
				mCurrentModeIcon[i].setImageDrawable(mModeSelectionIcon[target]
						.getDrawable());
			}
		}
	}
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		if (mCurrentModeBar != null) {
			...
			mCurrentModeBar.layout((l + padding),
					...
					(l + padding + iconWidth), (bottom - top));
		}
	}
}
