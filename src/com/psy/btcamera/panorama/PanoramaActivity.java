...
package com.psy.btcamera.panorama;

import com.psy.btcamera.ActivityBase;
import com.psy.btcamera.CameraDisabledException;
import com.psy.btcamera.CameraHardwareException;
import com.psy.btcamera.CameraHolder;
import com.psy.btcamera.Exif;
import com.psy.btcamera.MenuHelper;
import com.psy.btcamera.ModePicker;
import com.psy.btcamera.OnClickAttr;
import com.psy.btcamera.RotateDialogController;
import com.psy.btcamera.ShutterButton;
import com.psy.btcamera.Storage;
import com.psy.btcamera.Thumbnail;
import com.psy.btcamera.Util;
...
public class PanoramaActivity extends ActivityBase implements
	...
	private void setOrientationIndicator(int degree) {
		if (mSharePopup != null)
			mSharePopup.setOrientation(degree);
	}
	...
	public void onCreate(Bundle icicle) {
		...
		mPreparePreviewString = getResources().getString(
		...
		mDialogPanoramaFailedString = getResources().getString(
				R.string.pano_dialog_panorama_failed);
		...
		mMainHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case MSG_RESET_TO_PREVIEW_WITH_THUMBNAIL:
					onBackgroundThreadFinished();
					// If the activity is paused, save the thumbnail to the file
					// here.
					// If not, it will be saved in onPause.
					if (mPausing)
						saveThumbnailToFile();
					// Set the thumbnail bitmap here because mThumbnailView must
					// be accessed
					// from the UI thread.
	...
	public void onMosaicSurfaceCreated(final int textureID) {
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (!mPausing) {
					mSurfaceTexture
							.setOnFrameAvailableListener(PanoramaActivity.this);
				}
			}
		});
	}
	...
	public void runMosaicCapture() {
		...
		// Call preprocess to render it to low-res and high-res RGB textures.
		mMosaicView.preprocess(mTransformMatrix);
		// Lock the conditional variable to ensure the order of transferGPUtoCPU
		// and
		// mMosaicFrame.processFrame().
		mMosaicView.lockPreviewReadyFlag();
		// Now, transfer the textures from GPU to CPU memory for processing
		mMosaicView.transferGPUtoCPU();
		// Wait on the condition variable (will be opened when GPU->CPU transfer
		// is done).
	...
	public synchronized void onFrameAvailable(SurfaceTexture surface) {
		 ...
		 * and use synchronize. No OpenGL calls can be done here.
		 */
		...
		// Updating the texture should be done in the GL thread which
		// mMosaicView is attached.
		mMosaicView.queueEvent(new Runnable() {
			@Override
			public void run() {
				// Check if the activity is paused here can speed up the
				// onPause() process.
				if (mPausing)
					return;
	...
	public void startCapture() {
		...
		mShutterButton
				.setBackgroundResource(R.drawable.btn_shutter_pan_recording);
	...
	private void updateProgress(float panningRateXInDegree,
		...
		// Since we only support horizontal panning, we should display a warning
		// message
		...
		int angleInMajorDirection = (Math.abs(progressHorizontalAngle) > Math
				.abs(progressVerticalAngle)) ? (int) progressHorizontalAngle
	...
	private void createContentView() {
		...
		if (getRequestedOrientation() == ActivityInfo.SCREEN_ORIENTATION_PORTRAIT) {
			Rotatable[] rotateLayout = {
					...
					mRotateDialog, mCaptureIndicator,
					mModePicker, mThumbnailView };
		...
		runBackgroundThread(new Thread() {
			...
			public void run() {
				...
				if (jpeg == null) { // Cancelled by user.
					...
					// The panorama image returned from the library is
					// orientated based on the
					// natural orientation of a camera. We need to set an
					// orientation for the image
					// in its EXIF header, so the image can be displayed
					// correctly.
					// The orientation is calculated from compensating the
					// device orientation at capture and the camera orientation
					// respective to
					...
					Uri uri = savePanorama(jpeg.data, jpeg.width, jpeg.height,
							orientation);
					if (uri != null) {
						// Create a thumbnail whose width or height is equal or
						// bigger
						...
						int inSampleSize = Integer.highestOneBit(Math.max(
								widthRatio, heightRatio));
						mThumbnail = Thumbnail.createThumbnail(jpeg.data,
								orientation, inSampleSize, uri);
						Util.broadcastNewPicture(PanoramaActivity.this, uri);
					}
					mMainHandler
							.sendMessage(mMainHandler
									.obtainMessage(MSG_RESET_TO_PREVIEW_WITH_THUMBNAIL));
				}
			}
		});
		reportProgress();
	...
	private void showSharePopup() {
		...
		if (mSharePopup == null || !uri.equals(mSharePopup.getUri())) {
			// The orientation compensation is set to 0 here because we only
			// support landscape.
			mSharePopup = new SharePopup(this, uri, mThumbnail.getBitmap(),
					mOrientationCompensation, findViewById(R.id.frame_layout));
		}
	...
	private Uri savePanorama(byte[] jpegData, int width, int height,
		...
		if (jpegData != null) {
			String filename = PanoUtil.createName(
					...
					mTimeTaken);
			Uri uri = Storage.addImage(getContentResolver(), filename,
	...
	public MosaicJpeg generateFinalMosaic(boolean highRes) {
		...
		if (width <= 0 || height <= 0) {
			// TODO: pop up a error meesage indicating that the final result is
			// not generated.
