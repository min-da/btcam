...
import com.psy.btcamera.IconListPreference;
...
public class SecondLevelIndicatorControlBar extends IndicatorControl implements
	...
	// There are some views in the ViewGroup before adding the indicator
	// buttons,
	...
	public void onClick(View view) {
		dismissSettingPopup();
		mOnIndicatorEventListener
				.onIndicatorEvent(OnIndicatorEventListener.EVENT_LEAVE_SECOND_LEVEL_INDICATOR_BAR);
	}

	private int getTouchViewIndex(int x, int width) {
		// If the current touch location is on close icon and above.
		if (x > mCloseIcon.getLeft())
			return indexOfChild(mCloseIcon);

		// Calculate if the touch event is on the indicator buttons.
		int count = getChildCount();
		if (count == mNonIndicatorButtonCount)
			return -1;
	...
	public boolean dispatchTouchEvent(MotionEvent event) {
		...
		int width = getWidth();
		if (width == 0)
			return false; // the event is sent before onMeasure()
		if (x > width)
			x = width;
	...
	@Override
	public OtherSettingIndicatorButton addOtherSettingIndicator(
			Context context, int resId, String[] keys) {
		OtherSettingIndicatorButton b = super.addOtherSettingIndicator(context,
				resId, keys);
		b.setBackgroundResource(R.drawable.bg_pressed);
		b.setIndicatorChangeListener(this);
		return b;
	...
	public void onShowIndicator(View view, boolean showed) {
		...
		if (!showed && (mPopupedIndicator != view))
			return;
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		mDivider.layout(offsetX, padding,
				(offsetX + mDivider.getMeasuredWidth()), (height - padding));
