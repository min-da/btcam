...
public class ZoomControlWheel extends ZoomControl {
	...
	private static final int MAX_SLIDER_ANGLE = ZOOM_OUT_ICON_DEGREES
			- (HIGHLIGHT_DEGREES / 2);
	private static final int MIN_SLIDER_ANGLE = ZOOM_IN_ICON_DEGREES
			+ (HIGHLIGHT_DEGREES / 2);
	private static final int DEFAULT_SLIDER_POSITION = MAX_SLIDER_ANGLE;
	private static final float EDGE_STROKE_WIDTH = 6f;
	private static final double BUFFER_RADIANS = Math
			.toRadians(HIGHLIGHT_DEGREES / 2);
	private static final double SLIDER_RANGE = Math.toRadians(MAX_SLIDER_ANGLE
			- MIN_SLIDER_ANGLE);
	...
	public boolean dispatchTouchEvent(MotionEvent event) {
		if (!onFilterTouchEventForSecurity(event) || !isEnabled())
			return false;
		...
		case MotionEvent.ACTION_MOVE:
			performZoom((Math.toRadians(MAX_SLIDER_ANGLE) - mSliderRadians)
					/ SLIDER_RANGE);
			requestLayout();
		}
		return true;
	...
	private void layoutIcon(View view, double radian) {
		...
		view.layout(x - width / 2, y - height / 2, x + width / 2, y + height
				/ 2);
	}
	...
	private double getZoomIndexAngle() {
		if (mZoomMax == 0)
			return Math.PI;
		return Math
				.toRadians(MAX_SLIDER_ANGLE
						- (MAX_SLIDER_ANGLE - MIN_SLIDER_ANGLE) * mZoomIndex
						/ mZoomMax);
	}
	...
	private void drawArc(Canvas canvas, int startAngle, int sweepAngle,
		...
		mBackgroundRect.set((float) (mCenterX - radius),
				(float) (mCenterY - radius), (float) (mCenterX + radius),
				(float) (mCenterY + radius));
		mBackgroundPaint.setStrokeWidth(width);
		mBackgroundPaint.setStrokeCap(Paint.Cap.ROUND);
		mBackgroundPaint.setColor(color);
		canvas.drawArc(mBackgroundRect, startAngle, sweepAngle, false,
				mBackgroundPaint);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if ((startAngle + radians) > 0)
			radians = -startAngle;
		drawArc(canvas, startAngle, radians, mWheelRadius, TRAIL_COLOR,
				TRAIL_WIDTH);
		super.onDraw(canvas);
	}
