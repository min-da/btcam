...
public class ControlPanelLayout extends RelativeLayout {
	...
	protected void onMeasure(int widthSpec, int heightSpec) {
		...
		boolean isLandscape = (((Activity) getContext())
		...
		if (widthSpecSize > 0 && heightSpecSize > 0
			...
			Log.e(TAG,
					"layout_xxx of ControlPanelLayout should be wrap_content");
		}

		// The size cannot be smaller than minimum constraint.
		int minimumSize = (isLandscape) ? getMinimumWidth()
				: getMinimumHeight();
		...
		if (isLandscape) {
			widthSpec = MeasureSpec.makeMeasureSpec(measuredSize,
					MeasureSpec.EXACTLY);
		} else {
			heightSpec = MeasureSpec.makeMeasureSpec(measuredSize,
					MeasureSpec.EXACTLY);
		}
