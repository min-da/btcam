...
package com.psy.btcamera.ui;

import com.psy.btcamera.PreferenceGroup;
...
public class IndicatorControlWheel extends IndicatorControl implements
	...
	private static final double HIGHLIGHT_RADIANS = Math
			.toRadians(HIGHLIGHT_DEGREES);
	...
	// For second-level indicators, the indicators are located evenly between
	// start
	...
	public IndicatorControlWheel(Context context, AttributeSet attrs) {
		...
		HIGHLIGHT_COLOR = resources
				.getColor(R.color.review_control_pressed_color);
		HIGHLIGHT_FAN_COLOR = resources
				.getColor(R.color.review_control_pressed_fan_color);
	...
	public void initialize(Context context, PreferenceGroup group,
		...
		mStrokeWidth = Util
				.dpToPixel(IndicatorControlWheelContainer.STROKE_WIDTH);
	...
	private ImageView addImageButton(Context context, int resourceId,
			boolean rotatable) {
		ImageView view;
		if (rotatable) {
	...
	private int getTouchIndicatorIndex(double delta) {
		...
		if ((delta >= (mChildRadians[startIndex] - halfTouchSectorRadians))
			...
			int index = 0;
			if (mCurrentLevel == 1) {
				index = (int) ((delta - mChildRadians[startIndex]) / mSectorRadians[mCurrentLevel]);
				// greater than the center of ending indicator
				if (index > sectors)
					return (startIndex + sectors);
				// less than the center of starting indicator
				if (index < 0)
					return startIndex;
			}
			if (delta <= (mChildRadians[startIndex + index] + halfTouchSectorRadians)) {
				return (startIndex + index);
			}
			if (delta >= (mChildRadians[startIndex + index + 1] - halfTouchSectorRadians)) {
	...
	public boolean dispatchTouchEvent(MotionEvent event) {
		...
		if ((radius <= (mWheelRadius + mStrokeWidth))
			...
			if ((index != mPressedIndex) || (action == MotionEvent.ACTION_DOWN)) {
				if (mPressedIndex != -1) {
					injectMotionEvent(mPressedIndex, event,
							MotionEvent.ACTION_CANCEL);
				} else {
					// Cancel the popup if it is different from the selected.
					if (getSelectedIndicatorIndex() != index)
						dismissSettingPopup();
				}
	...
	private void rotateWheel() {
		...
		int expectedAngle = startAngle
				+ (totalDegrees * elapsedTime / ANIMATION_TIME);
		double increment = Math.toRadians(expectedAngle)
				- mChildRadians[mSecondLevelStartIndex];
		for (int i = 0; i < getChildCount(); ++i)
			mChildRadians[i] += increment;
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		mCenterX = right
				- left
				- Util.dpToPixel(IndicatorControlWheelContainer.FULL_WHEEL_RADIUS);
		...
		for (int i = 0; i < getChildCount(); ++i) {
			...
			// We still need to show the disabled indicators in the second
			// level.
			double radian = mChildRadians[i];
			double startVisibleRadians = mInAnimation ? mStartVisibleRadians[1]
					: mStartVisibleRadians[mCurrentLevel];
			double endVisibleRadians = mInAnimation ? mEndVisibleRadians[1]
			...
			if (view == mZoomControl) {
				...
				view.layout(x - width / 2, y - height / 2, x + width / 2, y
						+ height / 2);
			}
		}
	}
	...
	private void presetFirstLevelChildRadians() {
		...
		int startIndex = 0;
		if (mZoomControl != null) {
			mChildRadians[startIndex++] = Math
					.toRadians(MAX_ZOOM_CONTROL_DEGREES);
		}
		if (mCameraPicker != null) {
			mChildRadians[startIndex++] = Math
					.toRadians(FIRST_LEVEL_START_DEGREES);
		}
	...
	private void presetSecondLevelChildRadians() {
		...
		double sectorDegrees = ((SECOND_LEVEL_END_DEGREES - SECOND_LEVEL_START_DEGREES) / sectors);
		...
		mTouchSectorRadians[1] = Math.min(HIGHLIGHT_RADIANS,
				Math.toRadians(sectorDegrees));
	...
	protected void onDraw(Canvas canvas) {
		...
		if (selectedIndex >= 1) {
			...
			mBackgroundRect.set(mCenterX - innerR, mCenterY - innerR, mCenterX
					+ innerR, mCenterY + innerR);
			fanPath.arcTo(mBackgroundRect, -degree + HIGHLIGHT_DEGREES / 2,
					-HIGHLIGHT_DEGREES);
			mBackgroundRect.set(mCenterX - outerR, mCenterY - outerR, mCenterX
					+ outerR, mCenterY + outerR);
			fanPath.arcTo(mBackgroundRect, -degree - HIGHLIGHT_DEGREES / 2,
					HIGHLIGHT_DEGREES);
			fanPath.close();
		...
		if (mTimeLapseInterval != 0) {
			...
			if (numberOfFrames > mNumberOfFrames) {
				...
				sweepAngle = timeDelta % mTimeLapseInterval * 360f
						/ mTimeLapseInterval;
			}

			canvas.drawArc(mBackgroundRect, 0, sweepAngle, false,
					mBackgroundPaint);
			invalidate();
		}
	...
	public void setEnabled(boolean enabled) {
		...
		if (mCurrentMode == MODE_VIDEO) {
			mSecondLevelIcon.setVisibility(enabled ? View.VISIBLE
					: View.INVISIBLE);
	...
	public void enableZoom(boolean enabled) {
		if (mZoomControl != null)
			mZoomControl.setEnabled(enabled);
	}

	public void onTouchOutBound() {
		dismissSettingPopup();
		if (mPressedIndex != -1) {
			injectMotionEvent(mPressedIndex, mLastMotionEvent,
					MotionEvent.ACTION_CANCEL);
			mPressedIndex = -1;
			invalidate();
		}
