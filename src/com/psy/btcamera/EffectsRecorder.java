...
public class EffectsRecorder {
	...
	public static boolean isEffectSupported(int effectId) {
		...
		case EFFECT_GOOFY_FACE:
			return Filter
					.isAvailable("com.google.android.filterpacks.facedetect.GoofyRenderFilter");
		case EFFECT_BACKDROPPER:
			return Filter
					.isAvailable("android.filterpacks.videoproc.BackDropperFilter");
		default:
			return false;
		}
	}
	...
	public void setPreviewDisplay(SurfaceHolder previewSurfaceHolder,
		...
		case STATE_RECORD:
			throw new RuntimeException(
					"setPreviewDisplay cannot be called while recording!");
		case STATE_RELEASED:
			throw new RuntimeException(
					"setPreviewDisplay called on an already released recorder!");
		default:
			break;
		}
	...
	private void setFaceDetectOrientation() {
		if (mCurrentEffect == EFFECT_GOOFY_FACE) {
			...
			Filter metaRotateFilter = mRunner.getGraph()
					.getFilter("metarotate");
	...
	/**
	 * Passes the native orientation of the Camera app (device dependent) to
	 * allow for correct output aspect ratio. Defaults to portrait
	 */
	...
	private void initializeFilterFramework() {
		...
		mGraphEnv.addReferences("textureSourceCallback", mSourceReadyCallback,
				"recordingWidth", mProfile.videoFrameWidth, "recordingHeight",
				mProfile.videoFrameHeight, "recordingProfile", mProfile,
	...
	private synchronized void initializeEffect(boolean forceReset) {
		if (forceReset || mCurrentEffect != mEffect
			...
			if (mLogVerbose) {
				Log.v(TAG, "Effect initializing. Preview size " + mPreviewWidth
						+ ", " + mPreviewHeight);
			}

			mGraphEnv.addReferences("previewSurface",
					mPreviewSurfaceHolder.getSurface(), "previewWidth",
			...
			mOldRunner = mRunner;
			mRunner = mGraphEnv.getRunner(mGraphId,
					GraphEnvironment.MODE_ASYNCHRONOUS);
			...
			if (mState == STATE_PREVIEW || mState == STATE_STARTING_PREVIEW) {
				...
				try {
					...
					throw new RuntimeException(
							"Unable to connect camera to effect input", e);
				}
				mOldRunner.stop();
			}
		}
	...
	public synchronized void startPreview() {
		...
		if (mPreviewSurfaceHolder == null) {
			if (mLogVerbose)
				Log.v(TAG,
						"Passed a null surface holder; waiting for valid one");
		...
		public void onSurfaceTextureSourceReady(SurfaceTexture source) {
			...
			synchronized (EffectsRecorder.this) {
				...
				if (mState == STATE_RELEASED) {
					// EffectsRecorder has been released, so don't touch the
					// camera device
					// or anything else
					if (mLogVerbose)
						Log.v(TAG,
								"Ready callback: Already released, skipping.");
					return;
				}
				if (source == null) {
	...
	private OnRecordingDoneListener mRecordingDoneListener = new OnRecordingDoneListener() {
		// Forward the callback to the VideoCamera object (as an asynchronous
		// event).
		@Override
	...
	public synchronized void startRecording() {
		...
		recorder.setInputValue("audioSource",
		...
		public void onRunnerDone(int result) {
			synchronized (EffectsRecorder.this) {
				...
				if (mOldRunner != null) {
					...
					GLEnvironment glEnv = mGraphEnv.getContext()
							.getGLEnvironment();
				...
				if (mState == STATE_PREVIEW || mState == STATE_STARTING_PREVIEW) {
					...
					if (mLogVerbose)
						Log.v(TAG,
								"Previous effect halted, starting new effect.");
					tryEnable3ALocks(false);
					mRunner.run();
					...
					// STATE_RELEASED - camera will be/has been released as
					// well, do nothing.
				}
			}
		}
